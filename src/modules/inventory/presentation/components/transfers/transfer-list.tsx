...
import { Plus, Search, ArrowRightLeft, MoreHorizontal, Play, CheckCircle, XCircle, Ban, Eye } from "lucide-react";
                  ...
                  <tbody>
                    {data.data.map((transfer) => (
                      <tr key={transfer.id} className="border-b">
                        ...
                        <td className="py-4 pr-4">
                          <div>
                            <p className="font-medium">
                              {t("list.itemsCount", { count: transfer.totalItems })}
                            </p>
                            {transfer.lines[0] && (
                              <p className="text-sm text-muted-foreground">
                                {transfer.lines[0].productName}
                                {transfer.totalItems > 1 && ` +${transfer.totalItems - 1}`}
                              </p>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="py-4 pr-4 font-medium">{transfer.totalQuantity}</td>
                                  ...
                                  <div className="absolute right-0 z-10 mt-2 w-48 rounded-md border bg-popover p-1 shadow-md">
                                    ...
                                    {transfer.canCancel && (
                                      <button
                                        ...
                                        onClick={() => handleUpdateStatus(transfer.id, "CANCELED")}
                                        disabled={updateStatus.isPending}
                                      >
